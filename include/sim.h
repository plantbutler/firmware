...
/* THE FAKE'S CLOCK CONTRACT:
     hal_millis()   advances the rig by exactly 1 ms and runs every model (watchdog,
                    pump-on time, the flow and screw edges), then returns the millisecond
                    it has just reached. That is what lets a bounded spin loop — the alive
                    probe, safety_wait_ms(), the dose loop — make progress on a host with
                    no real time.
...
/* The whole-word PmnPFS shape, mirrored so the host can assert it:
...
void     sim_reset(bool warm);             /* re-enter setup(); warm keeps .noinit */
void     sim_advance(uint32_t ms);
void     sim_set_float(bool ok);           /* D5; consumed by the debounce */
/* Each character is consumed by ONE hal_pin_read(PIN_HALL_FLOAT): '1' == OK (the pin reads
   ...
   grants one sample and fails the next two -- exactly the waterline bounce.
   sim_set_float() clears any pattern. */
void     sim_set_float_pattern(const char *pattern);
void     sim_set_flow_ml_s(uint16_t ml_s); /* the pump's delivery rate */
void     sim_flow_storm(uint32_t hz);      /* an edge storm on D2 */
...
/* Deliver exactly n flow pulses immediately after the ON write, then nothing, ever. The
   stall rule's fixture: a dose that flowed briefly and stopped. */
void     sim_set_flow_burst_pulses(uint32_t n);
/* Called ONCE, from inside the next pump assertion (before hal_pump_write(true) returns):
   the fixture for a case that has to observe net_poll() from a point only the dose loop
   can reach, because the dose loop blocks and safety_dosing() has no setter by design. */
void     sim_on_pump_on(void (*cb)(void));
/* Scheduled injectors, both measured from the next pump-on: apply the change when the
   fake's clock reaches `ms` into the dose. */
void     sim_set_float_at_ms(uint32_t ms, bool ok);
void     sim_serial_rx_at_ms(uint32_t ms, const char *s);
void     sim_set_i2c_fail(bool fail);      /* every expander transfer fails */
void     sim_set_mux_stuck(bool stuck);    /* every channel returns the canary's value */
void     sim_set_stall(bool on);           /* the screw stops pulsing while driven */
void     sim_set_leak(bool on);            /* pulses with D6 off */
void     sim_wdt_stop(void);               /* the counter FREEZES */
void     sim_wdt_rate_hz(uint32_t hz);     /* the counter moves, at this rate */
void     sim_noinit_clobber(void);         /* scramble the .noinit struct */
...
/* The clock injector. Body: g_us = ms * 1000u; g_ms = ms; g_ms_frac_us = 0u; one bounded
   tick_models_() call; nothing else. The frac reset is not optional: g_ms is its own
   counter, not derived from g_us, so a stale sub-millisecond remainder from before the
   jump would otherwise carry into the value hal_delay_us() next reports. */
void     sim_set_clock_ms(uint32_t ms);

/* Move the fake's break, so the per-report heap check has something to fail against;
   otherwise hal_heap_break() returns a fixed address comfortably inside the margin. */
...
typedef enum {
  ...
  SIM_EV_SERVO,
  SIM_EV_ADC,
  SIM_EV_SCREW         /* one per accepted screw pulse */
} sim_ev_kind_t;
...
uint16_t    link_fake_write_count(void);    /* socket writes since link_fake_reset() */
